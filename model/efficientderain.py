...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.convolution = nn.Conv2d(512, 512, 4, 2)

        self.view = View((-1, 512 * 1 * 1))
        self.encoder_linear = nn.Linear(512, 128)
        # 6~8层要先上采样再卷积
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        convolution_output = self.convolution(conv5)

        print(convolution_output.shape)
        view_output = self.view(convolution_output)
        linear_output = self.encoder_linear(view_output)
