...
if __name__ == "__main__":
    ...
    from data.deraining import DataLoaderTrain
    # config file
    config = Config({
        # device
        "GPU_ID": "0",
        ...
        "db_path": "D:\\intern\\IQA\\iqa-db\\LIVE_dataset\\databaserelease2",  # root of dataset
        "snap_path": "./weights/KADID",  # path for saving weights
        "txt_file_name": "D:\\intern\\IQA\\iqa-db\\all_data_csv\\LIVE.txt.csv",
        "train_size": 0.8,
        "scenes": "all",
    ...
    d_img = torch.rand(2, 3, 256, 256).to(config.device)
    enc_inputs = torch.ones(config.batch_size, config.n_enc_seq).to(config.device)
    dec_inputs = torch.ones(config.batch_size, config.n_dec_seq).to(config.device)

    model = MPRNet(in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3,
         reduction=4, bias=False).to(config.device)
